...
| Inferred provider gates could tell the user to click a generic "resume button" even though the control room button says "I finished this step", and token gates still mentioned hidden capture prompts. | Inferred follow-me gates now use the visible control-room button label and direct token/key gates to the safe VM clipboard Capture button. |

...
| `fusekit detonate` / remote detonation | Deletes worker/tmp state, browser/visual/OpenClaw scratch state, FuseKit-controlled transient logs, uploaded app archives, passphrase files, and remote OCI resources. | Detonation preflight requires encrypted/redacted survivor artifacts, safe verification report, and rollback metadata before trusting cleanup; workspace detonation receipts must prove the remote worker, OCI VM, and every standard FuseKit-created network resource class were deleted or name the missing classes; live acceptance rejects leftover plaintext worker, browser, visual, provider-auth, or gateway/control-room scratch. |

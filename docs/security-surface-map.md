...
| `fusekit authorize` | Captures approved provider secrets into the encrypted vault. | Public guided runs use VM clipboard Capture buttons for approved env targets; CLI-only fallback can use a non-echoing prompt or env handoff. Raw secrets are redacted from output, logs, receipts, and control-room payloads. |
...
- The live control-room provider-gate launcher accepts only executable
  ...
  passing it through the `DISPLAY` environment.

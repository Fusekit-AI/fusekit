...
| Rehearsal Review | Required proof that every human action was expected, visible in the control room, and compared against launcher/VNC instructions before public recording readiness. | The review is redacted durable evidence. It fails if the user needed a host browser, terminal command, side channel, unsupported manual step, or untracked provider interpretation. |
...
| Disposable Workers, Durable State | Required operational model. The VM is replaceable during the run and destroyed at the end. | Resume readiness must prove encrypted/redacted state is enough to recreate the worker. If host-machine or VM-local plaintext state is required, the run is not launch-ready; the durable detonation scope must explicitly carry `host_machine_state_required=false`. |
| Audit-First UX | Required user-facing ledger for credential capture, DNS writes, provider actions, human approvals, errors, and cleanup. | Audit entries must be plain-language and redacted. The detonation receipt must name every worker/OCI cleanup category, including boot volume deletion, ephemeral public IP release, and every failed deletion key. |

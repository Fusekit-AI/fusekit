...
def test_hosted_readiness_rejects_malformed_managed_price_id() -> None:
    settings = HostedSettings(
        public_origin="https://fusekit.snowmanai.org",
        github_app_id="12345",
        github_app_slug="fusekit-launcher",
    ...
    assert status == "200 OK"
    assert payload["ready"] is False
    assert payload["invalid"] == ["stripe_price_id_required_for_managed_runs"]
    assert payload["payment"]["enabled"] is False
    assert payload["payment"]["price_configured"] is False
    assert managed["launchable"] is False
...
def test_hosted_managed_payment_dispatch_uses_job_price_binding_after_rotation() -> None:
    state = create_hosted_state_token(
        STATE_SECRET,
        return_path="/",
        nonce="nonce-for-hosted-state",
    ...
    stripe_opener = FormSequenceOpener(
        [
            {
                "id": "cs_test_rotated_price",
                "object": "checkout.session",
                "url": "https://checkout.stripe.com/c/pay/cs_test_rotated_price",
                "status": "open",
                "payment_status": "unpaid",
                "mode": "payment",
            }
    ...
    job_price_hash = job.payment_price_id_hash
    rotated_price_hash = _payment_public_hash("price_rotated_managed_run")
    assert status == "200 OK"
    assert job_price_hash == _payment_public_hash("price_managed_run")
    ...
    stripe_opener.payloads.append(
        {
            ...
            "metadata": {
                ...
                "stripe_price_id_hash": job_price_hash,
                "price_label_hash": _payment_public_hash(job.payment_price_label),
            },
        }
    )
    rotated_settings = replace(settings, stripe_price_id="price_rotated_managed_run")
    ...
    started = json.loads(body.decode("utf-8"))
    dispatch_binding = started["worker_dispatch"]["dispatch_binding"]
    request_binding = dispatch_opener.bodies[0]["dispatch_binding"]

    assert status == "200 OK"
    assert dispatch_binding["stripe_price_id_hash"] == job_price_hash
    assert request_binding["stripe_price_id_hash"] == job_price_hash
    assert dispatch_binding["stripe_price_id_hash"] != rotated_price_hash
    assert request_binding["stripe_price_id_hash"] != rotated_price_hash
    assert len(dispatch_opener.requests) == 1
...
def test_hosted_payment_checkout_rejects_missing_checkout_url_before_pending_state() -> None:
    ...
    assert status == "502 Bad Gateway"
    _assert_public_payment_error(body, "payment_checkout_url_unavailable")
    assert "Location" not in headers
    assert settings.hosted_jobs[job_id].payment_status == "payment_required"
    assert len(stripe_opener.requests) == 1


def test_hosted_payment_checkout_rejects_unpublic_stripe_receipt_before_pending_state() -> None:
    state = create_hosted_state_token(
        STATE_SECRET,
        return_path="/",
        nonce="nonce-for-hosted-state",
        ...
        [
            {
                ...
                "metadata": {
                    "job_id": "hosted-job",
                    "lane": MANAGED_FUSEKIT_RUN_LANE,
                    "provider_token": "not-allowed-in-public-receipts",
                },
            },
        ]
    )
    ...
    assert status == "503 Service Unavailable"
    _assert_public_payment_error(body, "payment_not_ready")
    assert "Location" not in headers
    assert settings.hosted_jobs[job_id].payment_status == "payment_required"
    assert len(stripe_opener.requests) == 1


def test_hosted_payment_checkout_rejects_bad_controls_without_stripe_call() -> None:
    state = create_hosted_state_token(
        STATE_SECRET,
        return_path="/",
        nonce="nonce-for-hosted-state",
    ...
    status, _headers, body = _call(
        ...
        method="POST",
        query_string=f"job={job_token}",
        json_body={"control": checkout_control},
        settings=settings,
    )
    assert status == "400 Bad Request"
    _assert_public_payment_error(body, "missing_control")
...
def test_hosted_byo_oci_lane_starts_without_managed_worker_dispatch() -> None:
    ...
    status, _headers, body = _call(
        ...
        method="POST",
        query_string=f"job={job_token}",
        form_body={"control": "not-used-for-byo"},
        settings=settings,
    )
    assert status == "400 Bad Request"

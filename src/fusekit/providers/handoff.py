...
HANDOFFS: dict[str, ProviderHandoff] = {
    ...
    "resend": ProviderHandoff(
        ...
        secret_steps=(
            ...
            (
                "An existing key row with Permission: Full access and Domain: All domains "
                "is not enough by itself; FuseKit needs the raw key value captured into "
                "the encrypted vault."
            ),
            (
                "If an existing key already has those selectors but the raw value is not "
                "available, create a new setup key because Resend does not reveal old key "
                "secrets again."
            ),
            (

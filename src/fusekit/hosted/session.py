...
HOSTED_STATE_MANAGED_PROOF_PURPOSE = "managed-checkout-proof"
...
class HostedLaunchState:
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        if self.managed_proof:
            payload["purpose"] = HOSTED_STATE_MANAGED_PROOF_PURPOSE
        return payload
...
def verify_hosted_state_token(
    ...
    *,
    ...
    if purpose and purpose != HOSTED_STATE_MANAGED_PROOF_PURPOSE:
        raise FuseKitError("Hosted launcher state token purpose is invalid.")
    return HostedLaunchState(
        nonce=nonce,
        issued_at=issued_at,
        return_path=_safe_return_path(return_path),
        managed_proof=purpose == HOSTED_STATE_MANAGED_PROOF_PURPOSE,
    )
